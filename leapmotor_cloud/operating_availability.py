...
def front_seat_ventilation_availability(snapshot, state, *, position, rudder,
                                      now, capability_max_age, state_max_age):
    require_aware(now)
    ...
    if state.observed_at is None:
        return Decision(A.UNKNOWN, 'state_timestamp_missing')
    age = now - state.observed_at
    if age < timedelta(0) or age > state_max_age:
        return Decision(A.UNKNOWN, 'state_not_fresh')
    ...
    return Decision(A.AVAILABLE, 'b10_front_ventilation_checks_passed_not_execution_permit')
